...
$member_ids = array_map('intval', array_filter($raw_ids));
...
class BoardingPDF extends FPDF {
    ...
    function Header() {
        ...
        // Título Principal
        $this->SetSetFont('Helvetica', 'B', 16);
    ...
    function SetSetFont($family, $style = '', $size = 0) {
...
$pdf->loggedUserName = $loggedUser['name'];
...
$wRg    = 38;
$wPhone = 42;
// Total de largura: 68 + 38 + 38 + 42 = 186mm (largura útil do A4 com margens 12mm)
...
foreach ($members as $index => $m) {
    // Alternar cor de fundo das linhas (zebra)
    if ($fill) {
        $pdf->SetFillColor(248, 250, 252);
    } else {
...
// Gerar e enviar o PDF no navegador (inline)
